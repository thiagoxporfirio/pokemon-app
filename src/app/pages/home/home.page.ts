...
export class HomePage implements OnInit {
  ...
  loadPokemons(limit = 151) {
    ...
    this.pokemonService.getPokemons(limit).pipe(
      catchError(error => {
    ...
    this.filteredPokemons.forEach((pokemon, index) => {
      if (!pokemon.types) {
        this.pokemonService.getPokemonDetails(pokemon.id.toString()).subscribe(
          ...
          error => console.error(`Error loading details for ${pokemon.name}:`, error)
        );
      }
    });
  }
  ...
  loadMorePokemon(event: any) {
    ...
    setTimeout(() => {
      ...
      this.hasMorePokemon = this.currentPage * this.pageSize < this.pokemons.length;
